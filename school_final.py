...
# label for time
label4 = Label(root, text="", font=("Ubuntu", 15), fg="red",bg="Light Blue")
label4.place(x=0, y=0)
# label for day of week
label5 = Label(root, text=" ", font=("Ubuntu", 15), fg="red",bg="Light Blue")
label5.place(x=0, y=25)

# Reading file of certain student
def id_fun():
    with open(id.get() + ".txt", "r") as f:
        return f.write(str(submit()))
...
def submit():
    ...
    if id.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
    ...
    if first_name.get() == "":
        ...
        messagebox.showinfo('Sorry', "All Fields Are Required")
    elif father_name.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
    elif mother_name.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
    elif date_of_birth.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
    elif Contact_number.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
    elif Date_of_admission.get() == "":
        messagebox.showinfo('Sorry', "All Fields Are Required")
...
def show_records():
    ...
    search_std['values'] = ("ID", "Class", "Show All")
    ...
    def delete():
        conn = sqlite3.connect('School_Management.db')
        c = conn.cursor()
        c.execute("DELETE from addresses WHERE student_id=" + (search_entry.get()))

        conn.commit()
        conn.close()

    # Function to get edited records
    def edit():
        conn = sqlite3.connect('School_Management.db')
        c = conn.cursor()
        id_student = search_entry.get()
        c.execute("""UPDATE addresses SET
                  ...
                  {'std_id': id_edit.get(),
                   ...
                   'middle': Middle_name_edit.get(),
                   'last': last_name_edit.get(),
                   'Class': Class_edit.get(),
                   'age': age_edit.get(),
                   'address': address_edit.get(),
                   'dob': date_of_birth_edit.get(),
                   'gender': gender_edit.get(),
                   'father': father_name_edit.get(),
                   'mother': mother_name_edit.get(),
                   'contact': Contact_number_edit.get(),
                   'e_mail': Email_address_edit.get(),
                   'doa': Date_of_admission_edit.get(),
    ...
    def update():
        ...
        update_record = Toplevel()
        update_record.title("Update Record")
        update_record.geometry('1400x800')
        update_record.configure(bg='Crimson')
        update_record.iconbitmap('icon.ico')

        Update_frame = Frame(update_record, bg='Sky Blue', bd=10, relief=RIDGE)
        Update_frame.place(x=20, y=100, width=1300, height=500)

        conn = sqlite3.connect('School_Management.db')
        c = conn.cursor()
        id_student = search_entry.get()
        c.execute("SELECT * FROM addresses WHERE student_id=" + id_student)
        student = c.fetchall()
        # Creating global variable for text box names
        global id_edit
        global first_name_edit
        global Middle_name_edit
        global last_name_edit
        global Class_edit
        global age_edit
        global address_edit
        global date_of_birth_edit
        global gender_edit
        global father_name_edit
        global mother_name_edit
        global Contact_number_edit
        global Email_address_edit
        global Date_of_admission_edit

        id_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        id_edit.place(x=1010, y=15)

        first_name_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        first_name_edit.place(x=130, y=65)

        Middle_name_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        Middle_name_edit.place(x=600, y=65)

        last_name_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        last_name_edit.place(x=1010, y=65)

        Class_edit = Entry(Update_frame, font='Cambria 15 italic', width=13, borderwidth=5)
        Class_edit.place(x=105, y=115)

        age_edit = Entry(Update_frame, font='Cambria 15 italic', width=9, borderwidth=5)
        age_edit.place(x=390, y=115)

        address_edit = Entry(Update_frame, font='Cambria 15 italic', width=36, borderwidth=5)
        address_edit.place(x=105, y=170)

        date_of_birth_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        date_of_birth_edit.place(x=1010, y=113)

        gender_edit = Entry(Update_frame, font='Cambria 15 italic', width=7, borderwidth=5)
        gender_edit.place(x=660, y=115)

        father_name_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        father_name_edit.place(x=590, y=220)

        mother_name_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        mother_name_edit.place(x=172, y=220)

        Contact_number_edit = Entry(Update_frame, font='Cambria 15 italic', width=18, borderwidth=5)
        Contact_number_edit.place(x=1053, y=220)

        Email_address_edit = Entry(Update_frame, font=('Cambria 15 italic', 14,), width=40, borderwidth=5)
        Email_address_edit.place(x=815, y=170)

        Date_of_admission_edit = Entry(Update_frame, font='Cambria 15 italic', width=22, borderwidth=5)
        Date_of_admission_edit.place(x=190, y=276)

        # create textLabels

        id_label = Label(Update_frame, font='Cambria 15 italic', text='Student ID', bg='black', fg='white',
                         relief=SUNKEN, bd=5)
        id_label.place(x=890, y=15)

        first_name_label = Label(Update_frame, text='First Name', font='Cambria 15 italic', bg='black',
                                 fg='white', relief=SUNKEN, bd=5)
        first_name_label.place(x=5, y=65)

        Middle_name_label = Label(Update_frame, text='Middle Name', font='Cambria 15 italic', bg='black',
                                  fg='white', relief=SUNKEN, bd=5)
        Middle_name_label.place(x=460, y=65)

        last_name_label = Label(Update_frame, font='Cambria 15 italic', text='Last Name', bg='black', fg='white',
                                relief=SUNKEN, bd=5)
        last_name_label.place(x=890, y=65)

        class_label = Label(Update_frame, text='Class', font='Cambria 15 italic', bg='black', fg='white',
                            relief=SUNKEN, bd=5)
        class_label.place(x=5, y=115)

        age_label = Label(Update_frame, font='Cambria 15 italic', text='Age', bg='black', fg='white',
                          relief=SUNKEN, bd=5)
        age_label.place(x=310, y=115)

        address_label = Label(Update_frame, font='Cambria 15 italic', text='Address', bg='black', fg='white',
                              relief=SUNKEN, bd=5)
        address_label.place(x=5, y=170)

        date_of_birth_label = Label(Update_frame, font='Cambria 15 italic', text='Date Of Birth(*in AD)',
                                    bg='black', fg='white', relief=SUNKEN, bd=5)
        date_of_birth_label.place(x=810, y=113)

        gender_label = Label(Update_frame, font='Cambria 15 italic', text='Gender', bg='black', fg='white',
                             relief=SUNKEN, bd=5)
        gender_label.place(x=565, y=115)

        father_name_label = Label(Update_frame, font='Cambria 15 italic', text="Father's Name ", bg='black',
                                  fg='white', relief=SUNKEN, bd=5)
        father_name_label.place(x=440, y=220)

        mother_name_label = Label(Update_frame, font='Cambria 15 italic', text="Mother's Name ", bg='black',
                                  fg='white', relief=SUNKEN, bd=5)
        mother_name_label.place(x=5, y=220)

        Contact_number_label = Label(Update_frame, font='Cambria 15 italic', text='Contact Number', bg='black',
                                     fg='white', relief=SUNKEN, bd=5)
        Contact_number_label.place(x=867, y=220)

        Email_label = Label(Update_frame, font='Cambria 15 italic', text='Email address', bg='black', fg='white',
                            relief=SUNKEN, bd=5)
        Email_label.place(x=677, y=170)

        Date_of_admission_label = Label(Update_frame, font='Cambria 15 italic', text='Date Of Admission',
                                        bg='black', fg='white', bd=5, relief=SUNKEN)
        Date_of_admission_label.place(x=5, y=275)

        for students in student:
            id_edit.insert(0, students[0])
            first_name_edit.insert(0, students[1])
            Middle_name_edit.insert(0, students[2])
            last_name_edit.insert(0, students[3])
            Class_edit.insert(0, students[4])
            age_edit.insert(0, students[5])
            gender_edit.insert(0, students[6])
            date_of_birth_edit.insert(0, students[7])
            address_edit.insert(0, students[8])
            mother_name_edit.insert(0, students[9])
            father_name_edit.insert(0, students[10])
            Email_address_edit.insert(0, students[11])
            Contact_number_edit.insert(0, students[12])
            Date_of_admission_edit.insert(0, students[13])
        # Creating save button to save edited records
        save_button = Button(Update_frame, text='SAVE', font=('Cambria 15 italic', 13, 'bold'), width=15, bd=5,
                             command=edit)
        save_button.place(x=50, y=350)

    # Creating a function to show records
    def show():
        if search_std.get() == "Show All":
            conn = sqlite3.connect('School Management.db')
            c = conn.cursor()
            c.execute("SELECT *, oid FROM addressA")
            ...
            conn.commit()
            conn.close()
    # Writting the updated status with previous one to know the changes
    if update_file == True:
        with open(id.get() + ".txt", "a") as f:
            return f.write(str(submit()))
    ...
    update_btn = Button(record, text='Update', font=('Cambria 15 italic', 14, 'bold'), width=10, bd=5, command=update)
    update_btn.place(x=950, y=10)
    delete_btn = Button(record, text='Delete', font=('Cambria 15 italic', 14, 'bold'), width=10, bd=5, command=delete)
